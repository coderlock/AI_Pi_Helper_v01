...
function initializeServerManagement() {
    ...
    if (dropdownContainer) {
        serverDropdown = new server_dropdown_1.ServerDropdown({
    ...
    serverModal = new server_modal_1.ServerModal({
 ...
 */
async function handleServerSelect(serverId) {
    try {
 ...
 * Connect to a server
 */
async function connectToServer(server, password) {
 ...
 */
async function handleLocalTerminal() {
    if (isSSHConnected) {
 ...
 */
function handleEditServer(serverId) {
...
function handleQuickConnect() {
    ...
    if (dialog) {
        ...
        const hostInput = document.getElementById('qc-host');
        hostInput?.focus();
    }
}
/**
 ...
 */
async function handleServerSave(data, id) {
    try {
        if (id) {
 ...
 * Initialize quick connect dialog
 */
function initializeQuickConnectDialog() {
    const dialog = document.getElementById('quick-connect-dialog');
 ...
 */
async function handleQuickConnectSubmit(closeDialog) {
 ...
 */
function showQuickConnectError(message) {
 ...
 */
function hideQuickConnectError() {
    const errorDiv = document.getElementById('qc-error');
 ...
 * Initialize password prompt dialog
 */
function initializePasswordPromptDialog() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        const savePassword = document.getElementById('prompt-save-password').checked;
        if (!serverId)
            return;
        try {
            const server = await window.electronAPI.getServer(serverId);
 ...
 */
function showPasswordPromptError(message) {
 ...
 */
function hidePasswordPromptError() {
    const errorDiv = document.getElementById('password-prompt-error');
    errorDiv?.classList.add('hidden');
}
/**
 ...
 */
function showPasswordPrompt(server) {
    const dialog = document.getElementById('password-prompt-dialog');
...
function updateConnectionStatus(status, label) {
    ...
    if (!connectionBadge || !connectionLabel)
        return;
